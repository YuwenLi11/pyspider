...
class Handler(BaseHandler):
    ...
    def save_detail(self, d_id, name, title, crawl_date, content):
        db = pymysql.connect("localhost", "root", "", "Disease", charset="utf8")
        try:
            cursor = db.cursor()
            sql = ' INSERT INTO detail(link_id, Name, Title, ds_add_date, ds_last_crawl_date, content) VALUES(%d,"%s","%s",now(),"%s","%s")' % (
            d_id, name, title, crawl_date, content)
            print(sql)
            cursor.execute(sql)
        except:
            db.rollback()
    ...
    def info_page(self, response):
        ...
        for each in response.doc('.skin_knw .bd').items():
            ...
            self.save_detail(d_id, name, title, crawl_date, content)
