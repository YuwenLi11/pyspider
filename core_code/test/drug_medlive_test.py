...
CONFIG_FILE_PATH = "/Users/JasonZhang/Desktop/Test_Web/drug_medlive_test.json"
...
class Handler(BaseHandler):
    ...
    def index_page(self, response):
        ...
        # if not on detail webpages
        if plv < self.max_plv:
            # Go to next level webpages (nodes)
            for each_css in self.total_css[(plv - 1)]['link']:
                for each in response.doc(each_css).items():
                    self.crawl(each.attr.href, callback=self.index_page, save={'wsid': wsid, 'qid': qid, 'plv': plv},
                               fetch_type=self.fetch_method, allow_redirects=True)
            ...
            # Jump level webpages (nodes)
            for each_level_nodes in self.total_css[(plv - 1)]['jump_link']:
                plv = each_level_nodes['plv']
                for each_node_css in each_level_nodes['node']:
                    for each in response.doc(each_node_css).items():
                        self.crawl(each.attr.href, callback=self.index_page,
