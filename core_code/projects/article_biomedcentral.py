...
CONFIG_FILE_PATH = "/data/apps/crawler/envi-pyspider/config_file/article_biomedcentral.json"
...
class Handler(BaseHandler):
    ...
    def save_details(self, wsid, wpid, details_names, details_values, details_types):
        ...
        try:
            ...
            details_values = str(details_values.replace("'", "^"))
    ...
    def index_page(self, response):
        ...
        if plv < self.max_plv:
            ...
            ### Method3: Direct URL picking and paging
                ...
                for each_css in self.total_css[(plv - 1)]['link']:
                   ...
                   function() {
                     setTimeout("$('.section-panel > .show > a').click()", 1000);
                     setInterval("document.querySelectorAll('.section-panel div > .show > a').click()", 5000); 
                     return("1000");
        ...
        ### If the page level is on detail page
                ...
                for each in response.doc(table_css).items():
                    ...
                    for row in BeautifulSoup(str(each), "lxml")(rows_css):
                        ...
                        if row("th") != "" and len(row("th")) >= 2:
                            ...
                            json_list.append(dict([[name_title, table_row_name], [value_title, table_row_value]]))
                        else:
            ...
            for each_pair in self.detail_pairs_css:
                ...
                all_value = []
                try:
                    if each_pair["keep_format"] == "True":
                        item_value = response.doc(each_pair["value_css"]).remove('Table').text()
                except:
                    for x in response.doc(each_pair["value_css"]).items():
                ...
                if item_name == "":
                    ...
                    if item_value == "":
                        try:
                            item_value = response.doc(each_pair["alternative_value_css"]).text()
                        except:
                            pass
                    detail_id = self.save_details(wsid, qid, item_name, item_value, 0)
